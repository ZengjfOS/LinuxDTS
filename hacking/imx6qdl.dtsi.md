# i.MX6 imx6qdl.dtsi hacking

## 原文解析

* 加载相关.h头文件、[skeleton.dtsi](skeleton.dtsi.md)文件
...
* 节点简图
  * / 
    ...
    * soc 
      ...
      * ipu1: ipu@02400000 
      * imx_ion 
* 从节点简图可以看出，这里主要是注册具体的外部设备相关的控制器，不像在[imx6dl.dtsi](imx6dl.dtsi.md)中节点简图中，属于比较大范畴的描述。
...
```
    / {
        ...
        aliases {
            /**
             * i2c-gpio3是一个缩写，他是i2c_gpio的别名，这里同样也是使用了Labels and References机制
             */
            i2c-gpio3 = &i2c_gpio;
        };
    #endif
        /**
         * 参考：i.MX 6Solo/DualLite Applications Processor Reference Manual
         * 章节：Chapter 2 Memory Maps
         * page：214
         */
        intc: interrupt-controller@00a01000 {
            compatible = "arm,cortex-a9-gic";
            /**
             * #interrupt-cells和#address-cells概念是类似的，也就是说，用多少个u32来标识一个interrupt source。
             */
            #interrupt-cells = <3>;
            #address-cells = <1>;
            #size-cells = <1>;
            ...
            /**
             * <0x00a01000 0x1000>
             *     #address-cells = <1> --> 0x00a01000
             *     #size-cells = <1>    --> 0x1000
             */
            reg = <0x00a01000 0x1000>,          /* 00A01000 - 00A01FFFh Interrupt distributor */
                  <0x00a00100 0x100>;           /* 00A00100 - 00A001FFh Interrupt controller interfaces */
        };
        ...
        soc {
            ...
            /**
             * 具体每个HW block的interrupt source是如何物理的连接到interrupt controller的呢？在dts文件中是用
             * interrupt-parent这个属性来标识的，如果一个能产生中断的device node没有定义interrupt-parent的话，
             * interrupt-parent属性就是跟随parent node。因此，与其在所有的下游设备中定义interrupt-parent，不如
             * 统一在root node中定义了。
             * 
             * initc是一个lable，后续可以使用&来引用这个lable。
             * 
             * 没有定义interrupt-parent的interrupt controller就是root
             */
